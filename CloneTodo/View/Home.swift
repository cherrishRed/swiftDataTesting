    ...
    var body: some View {
        ...
        .overlay(alignment: .bottomTrailing, content: {
            Button(action: {
                ...
                .background(.darkBlue.shadow(.drop(color: .black.opacity(0.25), radius:
                5, x: 10, y: 10)), in: .circle)
            })
        })
        .onAppear(perform: {
                    ...
                    Text(day.date.format("dd"))
                        ...
                        .background(content: {
                            if isSameDate(day.date, date2: currentDate) {
                                Circle()
                                    .fill(.darkBlue)
                                    .matchedGeometryEffect(id: "TABINDICATIOR", in: animation)
                            }
    ...
    func paginateWeek() {
        ...
        if weekSlider.indices.contains(currentWeekIndex) {
            if let firstDate = weekSlider[currentWeekIndex].first?.date, currentWeekIndex == 0 {
                /// Inserting New Week at Oth Index and Removing Last Array Item
